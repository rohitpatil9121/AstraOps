...
console.log("AWS Insights:", data?.insights);
        ...
        setAwsSummary({
          ...
          severity: summary.severity || "stable",
          running_instances: Number(summary.running_instances ?? 0),
        });
    ...
    loadAwsMetrics();
    timer = setInterval(loadAwsMetrics, 60000);
  ...
  const healthColor = useMemo(() => {
    if (awsSummary.severity === "critical") return "text-red-400";
    if (awsSummary.severity === "warning") return "text-yellow-400";
    return "text-emerald-400";
  }, [awsSummary.severity]);

  const insightMessage = useMemo(() => {
    if (awsSummary.severity === "critical") {
      return "Critical AWS signal detected. Review EC2 health immediately.";
    }

    if (awsSummary.severity === "warning") {
      return "AWS metrics are trending upward. Check CPU and memory utilization.";
    }

    return "AWS environment looks stable. No immediate action required.";
  }, [awsSummary.severity]);

  const scalingMessage = useMemo(() => {
    if (awsSummary.severity === "critical") {
      return "Scale immediately or inspect failing EC2 instances.";
    }

    if (awsSummary.severity === "warning") {
      return "Monitor load closely and prepare for scaling.";
    }
          ...
          <section>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              <MetricCard
                ...
                value={`${awsSummary.cpu_usage}%`}
              />

              <MetricCard
                title="Memory Utilization"
                value={
                  awsSummary.memory_usage === null
                    ? "N/A"
                    : `${awsSummary.memory_usage}%`
                }
              />
              ...
              <MetricCard
                title="Active Alerts"
                value={awsSummary.alerts}
              />

              <MetricCard
                title="System Health"
                value={`${awsSummary.health_score}%`}
                color={healthColor}
              />
            </div>
          </section>
          ...
          <section className="space-y-4">
            <div>
              ...
              <p className="text-sm text-slate-400 mt-1">
                Realtime AWS CPU and memory utilization.
              </p>
            </div>
